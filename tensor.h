...
typedef struct {
    ...
    void* map_pointer;      // questo campo contiene il pointer alla mappatura cosi nella free_tensor la si può usare nella munmap; solo se filename
    size_t map_size;        // come sopra, serve per la munmap nel caso il tensore sia inizializzato con '{'
} Tensor;

// struct per il salvataggio dei file su disco con le operazioni '{' e '}'
typedef struct {
